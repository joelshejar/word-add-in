...
Office.onReady((info) => {
  if (info.host === Office.HostType.Word) {
    ...
    document.getElementById("delete-table").onclick = () => tryCatch(deleteTable);
    document.getElementById("cursor-paragraph").onclick = () => tryCatch(addParagraphToEndOfCursor);
    document.getElementById("insert-image").onclick = () => tryCatch(insertImageAtCursor);
    document.getElementById("insert-header").onclick = () => tryCatch(addToHeader);
    document.getElementById("insert-footer").onclick = () => tryCatch(addToFooter);
    document.getElementById("set-landscape").onclick = () => tryCatch(setLandscapeOrientation);
  }
...
async function insertTable() {
  await Word.run(async (context) => {
    ...
    const tableData = [
      ["Name", "ID", "Birth City"],
      ["Joel", "434", "Kochi"],
      ["Rajesh", "719", "Changannasseri"],
  ];

  const selection = context.document.getSelection();
  const table =selection.insertTable(3, 3, Word.InsertLocation.after, tableData);
  table.parentContentControl.tag = "YourTableID";
...
async function deleteTable() {
  await Word.run(async (context) => {
    // const secondParagraph = context.document.body.tables.getFirst().getNext();
    // secondParagraph.delete();
    // await context.sync();


    const tables = context.document.body.tables;

    // Load the properties for all the tables
    tables.load("items, items/tag");
  
    await context.sync();
  
    // The specific ID of the table you want to delete
    const specificID = "YourTableID";
  
    // Loop through all the tables and find the one with the specific ID
    tables.items.forEach((table:any) => {
      if (table.tag === specificID) {
        // Delete the table
        table.delete();
      }
    });
...
// Run this function to add a paragraph to the end of the cursor position in the document.
async function addParagraphToEndOfCursor() {
  await Word.run(async (context) => {
    const selection = context.document.getSelection();
    ...
    selection.insertText(textSample, Word.InsertLocation.end);
    await context.sync();
  });
}
...
async function addToHeader() {
  await Word.run(async (context) => {
    const section = context.document.sections.getFirst();
    const header = section.getHeader("Primary");
    const text = "Heading"
    header.insertText(text, Word.InsertLocation.end);
    await context.sync();
  });
}

async function addToFooter() {
  await Word.run(async (context) => {
    const section = context.document.sections.getFirst();
    const footer = section.getFooter("Primary");
    const text = "Footer"
    footer.insertText(text, Word.InsertLocation.end);
    await context.sync();
  });
}
...
async function setLandscapeOrientation() {
  await Word.run(async (context) => {
    ...
    //   section.pageOrientation = Word.PageOrientation.landscape;
    // }
